...
while l<no_of_iter
    ...
    if(l~=0)
    	for i = 1:n
    	   %% Linearized Bhattacharyya constraint	
    	   ((x(i) - Ox_1(i))^2/mean_sig(1,1) + (y(i) - Oy_1(i))^2/mean_sig(2,2) + (z(i) - Oz_1(i))^2/mean_sig(3,3) + 2*(x(i) - Ox_1(i))*(Px(i) -  x(i))/mean_sig(1,1) + 2*(y(i) - Oy_1(i))*(Py(i) -  y(i))/mean_sig(2,2) + 2*(z(i) - Oz_1(i))*(Pz(i) -  z(i))/mean_sig(3,3))/8.0 + log(det(mean_sig)/sqrt(det(drone_sig)*det(obs_sig)))/2.0 >= R
    	   ((x(i) - Ox_2(i))^2/mean_sig(1,1) + (y(i) - Oy_2(i))^2/mean_sig(2,2) + (z(i) - Oz_2(i))^2/mean_sig(3,3) + 2*(x(i) - Ox_2(i))*(Px(i) -  x(i))/mean_sig(1,1) + 2*(y(i) - Oy_2(i))*(Py(i) -  y(i))/mean_sig(2,2) + 2*(z(i) - Oz_2(i))*(Pz(i) -  z(i))/mean_sig(3,3))/8.0 + log(det(mean_sig)/sqrt(det(drone_sig)*det(obs_sig)))/2.0 >= R 
           ((x(i) - Ox_3(i))^2/mean_sig(1,1) + (y(i) - Oy_3(i))^2/mean_sig(2,2) + (z(i) - Oz_3(i))^2/mean_sig(3,3) + 2*(x(i) - Ox_3(i))*(Px(i) -  x(i))/mean_sig(1,1) + 2*(y(i) - Oy_3(i))*(Py(i) -  y(i))/mean_sig(2,2) + 2*(z(i) - Oz_3(i))*(Pz(i) -  z(i))/mean_sig(3,3))/8.0 + log(det(mean_sig)/sqrt(det(drone_sig)*det(obs_sig)))/2.0 >= R 
    	end
    end
    %%%%%%%% Constraints %%%%%%%%
    Vx >= 0;
