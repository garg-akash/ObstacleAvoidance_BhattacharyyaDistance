...
while l<no_of_iter
    ...
    if(l~=0)
    	for i = 1:n
    	   ...
    	   ((x(i) - Ox(i))^2/mean_sig(1,1) + (y(i) - Oy(i))^2/mean_sig(2,2) + (z(i) - Oz(i))^2/mean_sig(3,3) + 2*(x(i) - Ox(i))*(Px(i) -  x(i))/mean_sig(1,1) + 2*(y(i) - Oy(i))*(Py(i) -  y(i))/mean_sig(2,2) + 2*(z(i) - Oz(i))*(Pz(i) -  z(i))/mean_sig(3,3))/8.0 + log(det(mean_sig)/sqrt(det(drone_sig)*det(obs_sig)))/2.0 >= R
