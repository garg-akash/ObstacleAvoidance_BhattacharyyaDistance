...
%% Drone Start and Destination variables
start_x = 0;
start_y = 0;
start_z = 4
dest_x = 12;
...
Ob_start=[0,start_y+4,start_z];
Ob_end=[dest_x, dest_y+4, dest_z];
...
drone_sig = diag([1^2 1^2 1^2]);

%% Defining obstalce uncertainty
obs_sig1 = diag([1^2 1^2 1^2]);
...
while l<no_of_iter
    ...
    if(l~=0)
    	for i = 1:n
    	  ...
    	  4.6052 >= ((x(i) - Ox(i))^2/mean_sig(1,1) + (y(i) - Oy(i))^2/mean_sig(2,2) + (z(i) - Oz(i))^2/mean_sig(3,3) + 2*(x(i) - Ox(i))*(Px(i) -  x(i))/mean_sig(1,1) + 2*(y(i) - Oy(i))*(Py(i) -  y(i))/mean_sig(2,2) + 2*(z(i) - Oz(i))*(Pz(i) -  z(i))/mean_sig(3,3))/8.0 + log(det(mean_sig)/sqrt(det(drone_sig)*det(obs_sig1)))/2.0 >= 1.1394
...
for i = 1:n
	...
	if i > 1	
		...
		surf(elD_x, elD_y, elD_z, 'FaceAlpha', 0.2, 'EdgeColor','none', 'FaceColor', [0.9 0.5 0.1]);
		hold on;
		...
		set(gca,'XLim', [floor(min([Px, Ox]))-1, floor(max([Px, Ox]))+1], 'YLim', [floor(min([Py, Oy]))-1, floor(max([Py, Oy]))+1], 'ZLim',[floor(min([Pz, Oz])) - 1, floor(max([Pz, Oz]))+1]); 
		hold on;
		title(strcat(' Blue line: Obstacle,', ' Red line: Drone path,', ' Total time = ', num2str(tot_time), ' 68% Conf < Bhattacharyya distance < 99% Conf '));
        az = 30;
        el = 60;
        view(az, el);
